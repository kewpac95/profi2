...
def compute_tanimoto(smiles, reference_fp):
...
class RLReward:
    ...
    def __call__(
        ...
        scores_list = []
        for smi in smilies:
            score = compute_tanimoto(smi, reference_fp)
            scores_list.append(score)

        scores = torch.tensor(scores_list, dtype=torch.float32, device=prior_nlls.device)
