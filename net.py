...
print("Ładuję net.py z:", __file__)
class MolecularVAE(nn.Module):
    def __init__(self, latent_size=32, alphabet_size=30, dropout=0, eps_coef=1):
        ...
        self.relu = nn.ReLU()
        self.selu = nn.SELU()
        # self.softmax = nn.Softmax(dim=1)  # USUŃ
...
class VaeLoss(nn.Module):
    ...
    def forward(self, x_hat, y, z_mean, z_logvar):
        # x_hat: (batch, seq_len, vocab_size) -- surowe logity z dekodera
        # y: (batch, seq_len) -- indeksy klas
        # Permute x_hat to (batch, vocab_size, seq_len) dla cross_entropy
        recon_loss = F.cross_entropy(
            x_hat.permute(0, 2, 1),  # (batch, vocab_size, seq_len)
            y,                       # (batch, seq_len)
            reduction="sum"
        )
        # KL divergence loss
